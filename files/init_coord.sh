...
sudo -u postgres sed -i "s/^#enforce_two_phase_commit =.*/enforce_two_phase_commit = 'on'/" "$postgres_home/$nodename/postgresql.conf"

sudo -u postgres /usr/local/pgsql/bin/pg_ctl start -D "$postgres_home/$nodename" -Z coordinator -l "$postgres_home/log/$nodename.log"
sleep 10
